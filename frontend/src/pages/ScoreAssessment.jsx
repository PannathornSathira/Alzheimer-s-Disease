...
export function ScoreAssessment() {
  ...
  const [selections, setSelections] = useState(() => {
    ...
    if (patientData?.cognitiveSeverityScore !== undefined && patientData?.cognitiveSeverityScore !== null) {
      return {
        cognitiveSeverity: patientData.cognitiveSeverityScore,
        vascularRisk: patientData.vascularRiskScore,
        behavioralSymptoms: patientData.behavioralSymptomsScore,
        functionalImpairment: patientData.functionalImpairmentScore,
        familyHistory: patientData.familyHistoryScore
      };
    }
    ...
    return {
      cognitiveSeverity: null,
      vascularRisk: null,
      behavioralSymptoms: null,
  ...
  useEffect(() => {
    if (patientData?.systemId) {
      localStorage.setItem(`score_${patientData.systemId}`, JSON.stringify(selections));
    }
  }, [selections, patientData?.systemId]);

  const totalScore = Object.values(selections).reduce((sum, val) => sum + (val !== null ? val : 0), 0);
  ...
  const getStratificationLabel = () => {
    ...
    const group = SCORE_CONFIG.stratification.find(s => totalScore >= s.minScore && totalScore <= s.maxScore);
    ...
    try {
      ...
      if (patientData.dbSessionId) {
        const response = await submitScore(patientData.dbSessionId, {
          cognitiveSeverityScore: selections.cognitiveSeverity,
          vascularRiskScore: selections.vascularRisk,
          behavioralSymptomsScore: selections.behavioralSymptoms,
          functionalImpairmentScore: selections.functionalImpairment,
          familyHistoryScore: selections.familyHistory
        });
      ...
      if (!finalAssignedArm) {
        ...
        finalAssignedArm = totalScore >= SCORE_CONFIG.minimumScoreForRandomization ? 'Drug Arm (Levetiracetam)' : null;
      }
  ...
  if (!patientData.systemId) {
    return (
      <Card className="mt-8 text-center p-8">
        <p className="text-danger mb-4">Error: No active System ID found. Please start from Registration.</p>
  ...
  return (
    <div className="max-w-[800px] mx-auto mt-6 mb-16 px-4">
      <LoadingOverlay isLoading={isLoading} message="Calculating & Randomizing..." />
      
      <Card className="shadow-lg border-slate-200 mt-2">
        <CardHeader>
          <CardTitle className="text-2xl text-slate-800">{SCORE_CONFIG.title}</CardTitle>
        ...
        <CardContent className="space-y-6">
          {SCORE_CONFIG.parameters.map(param => (
            <div key={param.key} className="p-4 bg-white border border-slate-200 rounded-lg hover:border-pink-200 transition-colors">
              <div className="mb-3 text-left">
                <label className="block text-[15px] font-semibold text-slate-800">{param.label}</label>
                {param.description && <p className="text-[13px] text-slate-500 mt-0.5">{param.description}</p>}
              </div>
              <div className="flex flex-wrap gap-3">
                {param.options.map(opt => {
                  ...
                  return (
                    <button
                      key={opt.id}
                      onClick={() => setSelections(prev => ({ ...prev, [param.key]: opt.points }))}
                    ...
                    >
                      {opt.label}
                      {isSelected && <span className="block text-xs mt-0.5 opacity-80">(+{opt.points})</span>}
                    </button>
                  );
                })}
              </div>
        ...
        <CardFooter className="flex-col items-stretch space-y-2">
          ...
          {allAnswered && !isEligible && (
            <div className="mb-2 text-center animate-fade-in-up">
              <p className="px-4 py-3 bg-[#fff1f2] text-[#be123c] border border-[#fda4af] rounded-lg text-[15px] font-bold shadow-sm">
                ⚠️ Patient is ineligible for randomization (Requires score ≥ {SCORE_CONFIG.minimumScoreForRandomization}).
              </p>
            </div>
          )}

          <div className="flex justify-between items-center bg-slate-50 border border-slate-200 rounded-lg p-6 w-full mb-4">
             <span className="text-lg font-bold text-slate-700 tracking-wider">Total Risk Score</span>
             <div className="flex flex-col items-end">
               <span className={`text-[40px] leading-none font-black ${totalScore > 0 ? 'text-[#E91E63]' : 'text-slate-300'}`}>
                 {totalScore}
               </span>
               {allAnswered && (
                 <span className={`text-sm font-bold mt-1 ${isEligible ? 'text-[#E91E63]' : 'text-slate-400'}`}>
          ...
          <div className="flex items-center justify-between w-full">
            ...
            >
              Calculate & Randomize
              <svg className="ml-2 w-5 h-5 opacity-90" fill="none" viewBox="0 0 24 24" stroke="currentColor">
